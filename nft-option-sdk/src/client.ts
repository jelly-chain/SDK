/**
 * NftOption SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface NftOptionConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class NftOptionSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: NftOptionConfig) {
    super(config, "NftOption");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "NftOption", status: "active", chainId: this.chainId };
  }
