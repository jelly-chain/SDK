/**
 * GmxPerp SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface GmxPerpConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class GmxPerpSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: GmxPerpConfig) {
    super(config, "GmxPerp");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "GmxPerp", status: "active", chainId: this.chainId };
  }
