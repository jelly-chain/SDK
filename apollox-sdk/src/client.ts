/**
 * Apollox SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface ApolloxConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class ApolloxSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: ApolloxConfig) {
    super(config, "Apollox");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Apollox", status: "active", chainId: this.chainId };
  }
