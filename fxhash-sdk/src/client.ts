/**
 * Fxhash SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface FxhashConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class FxhashSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: FxhashConfig) {
    super(config, "Fxhash");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Fxhash", status: "active", chainId: this.chainId };
  }
