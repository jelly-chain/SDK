/**
 * CurveStargate SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveStargateConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveStargateSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveStargateConfig) {
    super(config, "CurveStargate");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveStargate", status: "active", chainId: this.chainId };
  }
