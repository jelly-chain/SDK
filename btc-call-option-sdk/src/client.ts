/**
 * BtcCallOption SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface BtcCallOptionConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class BtcCallOptionSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: BtcCallOptionConfig) {
    super(config, "BtcCallOption");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "BtcCallOption", status: "active", chainId: this.chainId };
  }
