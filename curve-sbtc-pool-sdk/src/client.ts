/**
 * CurveSbtcPool SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveSbtcPoolConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveSbtcPoolSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveSbtcPoolConfig) {
    super(config, "CurveSbtcPool");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveSbtcPool", status: "active", chainId: this.chainId };
  }
