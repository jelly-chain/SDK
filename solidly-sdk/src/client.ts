/**
 * Solidly SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface SolidlyConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class SolidlySDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: SolidlyConfig) {
    super(config, "Solidly");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Solidly", status: "active", chainId: this.chainId };
  }
