/**
 * RaydiumClmm SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface RaydiumClmmConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class RaydiumClmmSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: RaydiumClmmConfig) {
    super(config, "RaydiumClmm");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "RaydiumClmm", status: "active", chainId: this.chainId };
  }
