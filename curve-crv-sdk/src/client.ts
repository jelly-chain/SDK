/**
 * CurveCrv SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveCrvConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveCrvSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveCrvConfig) {
    super(config, "CurveCrv");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveCrv", status: "active", chainId: this.chainId };
  }
