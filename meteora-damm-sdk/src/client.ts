/**
 * MeteoraDamm SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface MeteoraDammConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class MeteoraDammSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: MeteoraDammConfig) {
    super(config, "MeteoraDamm");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "MeteoraDamm", status: "active", chainId: this.chainId };
  }
