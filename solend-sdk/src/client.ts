/**
 * Solend SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface SolendConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class SolendSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: SolendConfig) {
    super(config, "Solend");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Solend", status: "active", chainId: this.chainId };
  }
