/**
 * Aerodrome SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface AerodromeConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class AerodromeSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: AerodromeConfig) {
    super(config, "Aerodrome");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Aerodrome", status: "active", chainId: this.chainId };
  }
