/**
 * UniswapNft SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface UniswapNftConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class UniswapNftSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: UniswapNftConfig) {
    super(config, "UniswapNft");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "UniswapNft", status: "active", chainId: this.chainId };
  }
