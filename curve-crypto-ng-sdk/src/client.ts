/**
 * CurveCryptoNg SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveCryptoNgConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveCryptoNgSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveCryptoNgConfig) {
    super(config, "CurveCryptoNg");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveCryptoNg", status: "active", chainId: this.chainId };
  }
