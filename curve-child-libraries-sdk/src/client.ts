/**
 * CurveChildLibraries SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveChildLibrariesConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveChildLibrariesSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveChildLibrariesConfig) {
    super(config, "CurveChildLibraries");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveChildLibraries", status: "active", chainId: this.chainId };
  }
