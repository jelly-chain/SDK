/**
 * EspnLive SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface EspnLiveConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class EspnLiveSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: EspnLiveConfig) {
    super(config, "EspnLive");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "EspnLive", status: "active", chainId: this.chainId };
  }
