/**
 * CurveEthPool SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveEthPoolConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveEthPoolSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveEthPoolConfig) {
    super(config, "CurveEthPool");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveEthPool", status: "active", chainId: this.chainId };
  }
