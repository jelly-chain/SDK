/**
 * Cricket SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CricketConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CricketSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CricketConfig) {
    super(config, "Cricket");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Cricket", status: "active", chainId: this.chainId };
  }
