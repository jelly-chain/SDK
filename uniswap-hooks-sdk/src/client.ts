/**
 * UniswapHooks SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface UniswapHooksConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class UniswapHooksSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: UniswapHooksConfig) {
    super(config, "UniswapHooks");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "UniswapHooks", status: "active", chainId: this.chainId };
  }
