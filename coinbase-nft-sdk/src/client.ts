/**
 * CoinbaseNft SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CoinbaseNftConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CoinbaseNftSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CoinbaseNftConfig) {
    super(config, "CoinbaseNft");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CoinbaseNft", status: "active", chainId: this.chainId };
  }
