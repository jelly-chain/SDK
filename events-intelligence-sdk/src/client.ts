/**
 * EventsIntelligence SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface EventsIntelligenceConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class EventsIntelligenceSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: EventsIntelligenceConfig) {
    super(config, "EventsIntelligence");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "EventsIntelligence", status: "active", chainId: this.chainId };
  }
