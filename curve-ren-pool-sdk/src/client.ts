/**
 * CurveRenPool SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveRenPoolConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveRenPoolSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveRenPoolConfig) {
    super(config, "CurveRenPool");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveRenPool", status: "active", chainId: this.chainId };
  }
