/**
 * SushiSwapRouter SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface SushiSwapRouterConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class SushiSwapRouterSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: SushiSwapRouterConfig) {
    super(config, "SushiSwapRouter");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "SushiSwapRouter", status: "active", chainId: this.chainId };
  }
