/**
 * NftTrader SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface NftTraderConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class NftTraderSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: NftTraderConfig) {
    super(config, "NftTrader");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "NftTrader", status: "active", chainId: this.chainId };
  }
