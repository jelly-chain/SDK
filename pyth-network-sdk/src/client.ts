/**
 * PythNetwork SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface PythNetworkConfig extends BaseSDKConfig {
  chainId?: ChainId;
}
...
export class PythNetworkSDK extends BaseSDK {
  ...
  constructor(config: PythNetworkConfig) {
    super(config, "PythNetwork");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "PythNetwork", status: "active", chainId: this.chainId };
  }
