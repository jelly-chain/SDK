/**
 * ElementV2 SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface ElementV2Config extends BaseSDKConfig {
  chainId?: ChainId;
}

export class ElementV2SDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: ElementV2Config) {
    super(config, "ElementV2");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "ElementV2", status: "active", chainId: this.chainId };
  }
