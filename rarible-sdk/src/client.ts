/**
 * Rarible SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface RaribleConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class RaribleSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: RaribleConfig) {
    super(config, "Rarible");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Rarible", status: "active", chainId: this.chainId };
  }
