/**
 * CurveMetaPool SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveMetaPoolConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveMetaPoolSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveMetaPoolConfig) {
    super(config, "CurveMetaPool");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveMetaPool", status: "active", chainId: this.chainId };
  }
