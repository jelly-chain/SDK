/**
 * BullBear SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface BullBearConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class BullBearSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: BullBearConfig) {
    super(config, "BullBear");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "BullBear", status: "active", chainId: this.chainId };
  }
