/**
 * BybitNft SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface BybitNftConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class BybitNftSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: BybitNftConfig) {
    super(config, "BybitNft");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "BybitNft", status: "active", chainId: this.chainId };
  }
