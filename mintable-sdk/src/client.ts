/**
 * Mintable SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface MintableConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class MintableSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: MintableConfig) {
    super(config, "Mintable");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Mintable", status: "active", chainId: this.chainId };
  }
