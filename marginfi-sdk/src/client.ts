/**
 * Marginfi SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface MarginfiConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class MarginfiSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: MarginfiConfig) {
    super(config, "Marginfi");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Marginfi", status: "active", chainId: this.chainId };
  }
