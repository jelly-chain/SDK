/**
 * Beets SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface BeetsConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class BeetsSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: BeetsConfig) {
    super(config, "Beets");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Beets", status: "active", chainId: this.chainId };
  }
