/**
 * OkxNft SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface OkxNftConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class OkxNftSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: OkxNftConfig) {
    super(config, "OkxNft");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "OkxNft", status: "active", chainId: this.chainId };
  }
