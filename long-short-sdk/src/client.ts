/**
 * LongShort SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface LongShortConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class LongShortSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: LongShortConfig) {
    super(config, "LongShort");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "LongShort", status: "active", chainId: this.chainId };
  }
