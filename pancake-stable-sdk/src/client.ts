/**
 * PancakeStable SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface PancakeStableConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class PancakeStableSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: PancakeStableConfig) {
    super(config, "PancakeStable");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "PancakeStable", status: "active", chainId: this.chainId };
  }
