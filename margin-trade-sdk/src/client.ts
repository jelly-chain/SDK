/**
 * MarginTrade SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface MarginTradeConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class MarginTradeSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: MarginTradeConfig) {
    super(config, "MarginTrade");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "MarginTrade", status: "active", chainId: this.chainId };
  }
