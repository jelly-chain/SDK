/**
 * Mintgate SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface MintgateConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class MintgateSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: MintgateConfig) {
    super(config, "Mintgate");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Mintgate", status: "active", chainId: this.chainId };
  }
