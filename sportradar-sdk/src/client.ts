/**
 * Sportradar SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface SportradarConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class SportradarSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: SportradarConfig) {
    super(config, "Sportradar");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Sportradar", status: "active", chainId: this.chainId };
  }
