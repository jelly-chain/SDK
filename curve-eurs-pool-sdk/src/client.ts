/**
 * CurveEursPool SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveEursPoolConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveEursPoolSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveEursPoolConfig) {
    super(config, "CurveEursPool");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveEursPool", status: "active", chainId: this.chainId };
  }
