/**
 * PancakeNft SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface PancakeNftConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class PancakeNftSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: PancakeNftConfig) {
    super(config, "PancakeNft");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "PancakeNft", status: "active", chainId: this.chainId };
  }
