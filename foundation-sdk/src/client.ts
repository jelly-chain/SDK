/**
 * Foundation SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface FoundationConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class FoundationSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: FoundationConfig) {
    super(config, "Foundation");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "Foundation", status: "active", chainId: this.chainId };
  }
