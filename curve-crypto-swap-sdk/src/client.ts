/**
 * CurveCryptoSwap SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveCryptoSwapConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveCryptoSwapSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveCryptoSwapConfig) {
    super(config, "CurveCryptoSwap");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveCryptoSwap", status: "active", chainId: this.chainId };
  }
