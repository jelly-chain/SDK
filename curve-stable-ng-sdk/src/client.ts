/**
 * CurveStableNg SDK - Protocol integration
 */
import { BaseSDK, type BaseSDKConfig, ChainId } from "@jellychain/sdk-core";

export interface CurveStableNgConfig extends BaseSDKConfig {
  chainId?: ChainId;
}

export class CurveStableNgSDK extends BaseSDK {
  readonly chainId: ChainId;
  
  constructor(config: CurveStableNgConfig) {
    super(config, "CurveStableNg");
    this.chainId = config.chainId || ChainId.ETHEREUM;
  }

  async getInfo(): Promise<any> {
    return { name: "CurveStableNg", status: "active", chainId: this.chainId };
  }
